//#pragma once
//// HermesPortal v1.3.1 (core, Option C + 7202 + SHM sink hook)
//// Umbrella header: common types, utilities, dispatcher, sink, declarations.
//
//#include <cstdint>
//#include <cstddef>
//#include <string>
...
//        if (s_extWriter) {
//            (void)s_extWriter(line);    // ignore failure; DROP-NEWEST is expected policy
//            return;
//        }
...
#pragma once
// HermesPortal core umbrella header (merged/updated).
// Keep existing content and additions for FeedType and parser API.
...
public:
    ...
    inline void sendLine(const std::string& line) {
        if (s_extWriter) { (void)s_extWriter(line); return; }
        if (s_consoleMirror) std::cout << line << "\n";
    }
private:
    inline static bool s_consoleMirror = true; // C++17 inline var
...
// ---------------- Feed selection ----------------
enum class FeedType { FO = 0, CM = 1 };

// Global debug flag for CM/LZO diagnostics (set from CLI)
...
class PacketParser {
public:
    explicit PacketParser(PacketDispatcher& d, FeedType feed = FeedType::FO) : disp_(d), feed_(feed) {}
    void parse(const char* buf, int len, ConsoleSink& out,
        InstrumentDirectory* instDir, const StrikeList& strikes);
    void setFeedType(FeedType f) { feed_ = f; }
    FeedType feedType() const { return feed_; }
private:
    // FO parsing kept in parseFO (original FO logic)
    void parseFO(const char* buf, int len, ConsoleSink& out,
        InstrumentDirectory* instDir, const StrikeList& strikes);

    // CM parsing implemented in Parser.cpp
    void parseCM(const uint8_t* buf, size_t len, ConsoleSink& out,
        InstrumentDirectory* instDir, const StrikeList& strikes);

    PacketDispatcher& disp_;
    FeedType feed_;
    // reusable buffer for CM decompression
    std::vector<uint8_t> cm_decomp_buf_;
};
...
// New CM handler declarations (CT, PN)
